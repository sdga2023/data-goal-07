...
# Sheet "accessabsolute" for Figure 2 --------------------------------------
...
pop.rururb.ok <- select(pop.rururb, iso3c, date, SP.RUR.TOTL, SP.URB.TOTL) %>%
  filter(date > 1999 & date <= 2021)
# Calculate population without electricity by urban and rural
...
access.pop.def <- mutate(access.pop, 
                         iso3c = if_else(iso3c %in% c("NAC", "ECS"), "RESTOFWORLD", iso3c)
                         ) %>%
  group_by(iso3c, date) %>%
...
# Sheet "access_emissions" for Figure 4 --------------------------------------

# Figure 4. Access vs emissions

# Import Emissions data downloaded from CAIT
emis.raw <- read.csv('../input/historical_emissions_ciat_updated.csv', na.strings = "FALSE")
# Download access (regions+world) with API
access.electricity.regions <- wb_data(indicator = c("EG.ELC.ACCS.ZS"), country = "regions_only")
...
# Download population (regions+world) with API
...
# Match countries to their region
